...
export default class PhotographersDatas {
	...
	static findAllPhotographers() {
		if (PHOTOGRAPHER_DATAS.length > 0) {
			return PHOTOGRAPHER_DATAS;
		} else {
			return false;
		}
	}
	...
	static getAllTags() {
		let allPhotographers = PHOTOGRAPHER_DATAS;
		let tagList = [];
		allPhotographers.forEach(photographer => {
			if (photographer.tags.length > 0) {
